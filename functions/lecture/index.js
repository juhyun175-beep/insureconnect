...
export const onRequestGet = async ({ env }) => {
  ...
  const desc = `보험설계사 영업교육·상품교육·세미나·웨비나 강의 ${items.length}건을 한 곳에서. 강사·일정·신청 정보를 확인하고 바로 신청하세요. 강사·교육기관은 무료로 강의를 등록할 수 있습니다.`;
...
<header class="h">
  ...
  <a class="h-cta" href="/api/auth/kakao/login">＋ 강사·교육기관이세요? 무료로 강의 등록 →</a>
</header>
${items.length
  ? `<div class="sec"><div class="grid">${cards}</div></div>`
  : `<div class="empty"><div class="empty-box">현재 등록된 강의가 없습니다. 잠시 후 다시 확인해주세요.<br><br><a href="/api/auth/kakao/login" style="color:#7c3aed;font-weight:700;text-decoration:none">강의 무료 등록하기 →</a></div></div>`}
