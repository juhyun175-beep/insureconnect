    ...
    `SELECT id, title, instructor, description, file_url, file_type, form_url, created_at,
            ...
            COALESCE((SELECT SUM(clicks) FROM ic_link_clicks_daily
                      WHERE company_name = 'lecture_' || ic_lectures.id
                        AND company_type = 'lecture_view'), 0) AS views
...
export const onRequestPost = async ({ request, env }) => handle(async () => {
  ...
  if (!isAdmin) {
    try {
      ...
      const basePrice = promoApplied ? 0 : finalPrice('lecture', rate);
      await env.DB.prepare(`UPDATE ic_lectures SET price=?, coupon_id=?, coupon_rate=? WHERE id=?`).bind(basePrice, usedId, rate, newId).run().catch(() => {});
      ...
      orderId = await createAdOrder(env, { ad_type: 'lecture', ad_id: newId, member_id: user ? user.id : null, submitter_name: submitterName, submitter_contact: submitterContact, base_price: AD_BASE.lecture, coupon_id: usedId, coupon_rate: rate, final_price: total, options_json: opt.keys.length ? JSON.stringify(opt.keys) : null, options_price: opt.total, promo_code: promoApplied ? LAUNCH_PROMO.code : null, consent_refund: cs.refund, consent_points: cs.points, consent_fail: cs.fail });
    } catch (_) {}
  }
