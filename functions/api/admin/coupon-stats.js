...
export const onRequestGet = async ({ request, env }) => handle(async () => {
  ...
  const revRecruit = await first(`SELECT COALESCE(SUM(COALESCE(total_price, price)),0) AS s, COUNT(*) AS n, COALESCE(SUM(CASE WHEN coupon_id IS NOT NULL THEN 1 ELSE 0 END),0) AS c FROM ic_recruitments WHERE price IS NOT NULL`);
  const revLecture = await first(`SELECT COALESCE(SUM(COALESCE(total_price, price)),0) AS s, COUNT(*) AS n, COALESCE(SUM(CASE WHEN coupon_id IS NOT NULL THEN 1 ELSE 0 END),0) AS c FROM ic_lectures WHERE price IS NOT NULL`);
  const revMeetup = await first(`SELECT COALESCE(SUM(COALESCE(total_price, price)),0) AS s, COUNT(*) AS n, COALESCE(SUM(CASE WHEN coupon_id IS NOT NULL THEN 1 ELSE 0 END),0) AS c FROM ic_meetings WHERE price IS NOT NULL`);
