...
export const onRequestPost = async ({ request, env }) => handle(async () => {
  ...
  const after = await env.DB.prepare(`SELECT points, ai_bonus FROM ic_members WHERE id = ?`).bind(user.id).first();
  return json({ ok: true, item: body.item, cost: item.cost, remaining: after?.points || 0, ai_bonus: after?.ai_bonus || 0 });
});
