...
export const onRequestGet = async ({ request, env }) => handle(async () => {
  ...
  const sql = company
    ? `SELECT * FROM ic_claim_forms WHERE company = ? ORDER BY created_at DESC LIMIT ?`
    : `SELECT * FROM ic_claim_forms ORDER BY created_at DESC LIMIT ?`;
...
export const onRequestPost = async ({ request, env }) => handle(async () => {
  ...
  const r = await env.DB.prepare(
    `INSERT INTO ic_claim_forms (company, title, file_url, file_type) VALUES (?, ?, ?, ?) RETURNING id`
