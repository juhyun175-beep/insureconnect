...
export const onRequestPost = async ({ request, env }) => handle(async () => {
  ...
  if (!isAdmin) {
    try {
      ...
      priceInfo = { base: AD_BASE.recruit, rate, price: total, options: opt.keys, options_price: opt.total };
      // v2.67.0: 주문/동의 기록(환불 정책)
      const cs = body.consent || {};
      orderId = await createAdOrder(env, { ad_type: 'recruit', ad_id: newId, member_id: user ? user.id : null, submitter_name: submitterName, submitter_contact: submitterContact, base_price: AD_BASE.recruit, coupon_id: usedId, coupon_rate: rate, final_price: total, options_json: opt.keys.length ? JSON.stringify(opt.keys) : null, options_price: opt.total, consent_refund: cs.refund, consent_points: cs.points, consent_fail: cs.fail });
    } catch (_) {}
  }
