...
export const onRequestPost = async ({ request, env }) => handle(async () => {
  ...
  if (!isAdmin) {
    try {
      ...
      const basePrice = promoApplied ? 0 : finalPrice('recruit', rate);
      await env.DB.prepare(`UPDATE ic_recruitments SET price=?, coupon_id=?, coupon_rate=? WHERE id=?`).bind(basePrice, usedId, rate, newId).run().catch(() => {});
      ...
      const opt = validateOptions('recruit', registrationOptionsForUser(body.options, user));
      const total = basePrice + opt.total;
      priceInfo = { base: AD_BASE.recruit, rate, price: total, options: opt.keys, options_price: opt.total, promo: { applied: promoApplied, code: LAUNCH_PROMO.code, remaining: Math.max(0, promo.remaining - (promoApplied ? 1 : 0)) } };
      // v2.67.0: 주문/동의 기록(환불 정책)
      const cs = body.consent || {};
      orderId = await createAdOrder(env, { ad_type: 'recruit', ad_id: newId, member_id: user ? user.id : null, submitter_name: submitterName, submitter_contact: submitterContact, base_price: AD_BASE.recruit, coupon_id: usedId, coupon_rate: rate, final_price: total, options_json: opt.keys.length ? JSON.stringify(opt.keys) : null, options_price: opt.total, promo_code: promoApplied ? LAUNCH_PROMO.code : null, consent_refund: cs.refund, consent_points: cs.points, consent_fail: cs.fail });
    } catch (_) {}
  }
