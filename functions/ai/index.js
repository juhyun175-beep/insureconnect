...
(function(){
  ...
  if(cqAsk){
    ...
    function _caseCard(c, i){var _num=(typeof i==='number')?'<span style="font-size:10.5px;font-weight:800;color:#64748b;margin-right:5px;">사례 '+(i+1)+'</span>':''; return '<div style="border:1px solid var(--border);border-radius:10px;padding:10px 12px;margin-bottom:8px;background:var(--bg,#f8fafc);"><div style="font-size:13px;color:var(--txt-hi,#0f172a);">'+_num+'<span style="font-size:10.5px;font-weight:800;color:#1a3de8;background:rgba(26,61,232,0.12);padding:1px 7px;border-radius:999px;">'+(_CAT[c.category]||'사례')+'</span> <b>'+_escc(c.disease||'')+'</b>'+(c.insurer?' · '+_escc(c.insurer):'')+(c.age?' · '+c.age+'세':'')+(c.elapsed_period?' · '+_escc(c.elapsed_period):'')+'</div>'+(c.result?'<div style="font-size:12.5px;color:#16a34a;font-weight:700;margin-top:4px;">→ '+_escc(c.result)+'</div>':'')+(c.summary?'<div style="font-size:12.5px;color:var(--txt-mid,#475569);margin-top:4px;line-height:1.55;">'+_escc(c.summary)+'</div>':'')+'</div>';}
