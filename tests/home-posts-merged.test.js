...
for (const marker of [
  ...
  '/api/recruitments?limit=25',
  '/api/lectures?limit=25',
  '/api/meetings?limit=25',
...
const fixture = [
  ...
  { kind: 'lecture', id: 2, created_at: '2026-07-03T00:00:00Z' },
  { kind: 'meetup', id: 3, created_at: '2026-07-02T00:00:00Z' },
];
for (let i = 4; i <= 31; i += 1) {
  fixture.push({
    kind: 'recruit',
    id: i,
    created_at: new Date(Date.parse('2026-07-01T00:00:00Z') - i * 86400000).toISOString(),
  });
}
const all = sandbox.__selectHomePosts(fixture, 'all');
assert.strictEqual(all.length, 25, 'all tab should render at most twenty-five cards (5x5)');
...
assert.strictEqual(sandbox.__selectHomePosts(fixture, 'recruit').length, 25, 'kind filter should cap at twenty-five cards');
