...
## [2.0.0] - 2026-05-19
- 신규: 홈 대시보드 4-컬럼 실시간 쇼케이스
  - 채용공고 + 인기 전산 + 인기 보험지식 + 인기 뉴스를 한 줄 나란히 배치
  - 채용공고 컬럼: 최신 3건 컴팩트 카드 (썸네일+제목+회사·날짜+NEW 배지)
  - 인기 전산 컬럼: 1·2·3위 순위 카드 (골드·블루·그레이 메달), 클릭 시 전산 페이지 이동
  - 인기 보험지식 컬럼: 1·2·3위 순위 카드, 클릭 시 해당 포스트 바로 열기
  - 인기 뉴스 컬럼: 1·2·3위 순위 카드, 클릭 시 해당 카드뉴스 모달 바로 열기
  - 기존 get_ic_top_items RPC 재활용 (추가 API 호출 없음)
  - 60초 자동 갱신 + 업데이트 시각 표시 (사이드바 모니터와 동시 갱신)
  - LIVE 배지 (블루 그라디언트), HIRING pulse 배지 적용
  - 데스크탑 4열 → 태블릿 2열 → 모바일 1열 반응형
  - 각 컬럼 "전체 →" 버튼으로 해당 페이지 직행
