...
## [1.5.3] - 2026-05-17
- 전역 푸터로 개편 (모든 페이지 공통 하단)
  - 홈 대시보드에만 있던 푸터를 제거하고 JS로 모든 페이지(.content[id^="page-"])에 자동 삽입
  - 디자인 개선: 약간 진한 배경(rgba(12,31,184,0.02)) + 위쪽 구분선 + 더 큰 패딩
  - 링크 hover 시 라운드 배경 강조
  - 정책 링크 5개: 서비스 소개 · 이용약관 · 개인정보처리방침 · 면책조항 · 운영자 문의
  - 카피라이트 + 면책 안내문 + 이메일 메타 정보 추가
  - 모바일 압축 스타일 적용 (≤480px)
- 결과: 8개 페이지(home/latest/jeonsan/payment/newsletter/claimform/recruit/tools) 모두 일관된 푸터
