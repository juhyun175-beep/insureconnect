...
## [2.137.0] - 2026-07-23
### Changed (홈 수익구조 재편: 제휴 파트너 존·B2B 문의·매체 고지)
- `ic_partner_cards` D1 테이블과 `/api/partners` 관리자 운영형 CRUD를 추가했습니다. 활성 파트너 카드는 홈 제휴 서비스 존에 AD 표기와 함께 노출되고, 클릭은 기존 link-click 집계로 기록됩니다.
- 홈 하단에 B2B 광고·제휴 문의 스트립을 추가하고, 푸터에 광고 매체 고지와 광고·제휴 앵커 링크를 추가했습니다.
- `about.html`, `contact.html`, `terms.html`, `disclaimer.html`의 이전 운영 문구를 현행 운영·광고 구조에 맞게 정비하고 직접 게재 광고(AD) 조항을 반영했습니다.
...
- `index.html`: 공고 노출 상한을 `HOME_POST_LIMIT`(25) 상수로 분리하고, 카테고리 칩 단독 선택 시에도 25건이 차도록 채용·강의·모임 API 요청 limit을 8→25로 상향. featured 우선 정렬·카테고리 인터리브 규칙과 추천공고(`ic-featured`) 섹션은 변경 없음.
- `tests/home-posts-merged.test.js`: 전체 탭·카테고리 필터의 25건 상한 회귀 검증으로 갱신.

## [2.133.1] - 2026-07-19
### Changed (낡은 테스트 정리 및 현행 회귀 커버리지 이전)

#### Removed

- 구 옵션별 개별 가격 상수 및 개별 주문 컬럼을 검사하던 `ad-options-pricing.test.js` 제거
- 제거된 `main_featured` 시스템 테스트 제거
- 직접 카카오 발송 방식의 레거시 활성화 테스트 제거
- 승인 API 직접 홈배너 이행을 검사하던 레거시 테스트 제거

#### Changed

- 여름 시즌 CTA 테스트를 상시 `내 공고 직접 등록` CTA 회귀 테스트로 교체
- `home_banner7` 이행 테스트를 `fulfillment-red-options.test.js`로 이전
- 홈 배너 캠페인의 실제 `ic_site_config` 저장 및 재승인 멱등성 검증 추가
- 환불 주문의 상태 보존 회귀 테스트 추가
- 계좌이체 신청 버튼 및 관리자 입금확인 문구 현행화
- 공용 SSR 셸 도입 이후 위젯과 CTA 푸터 조립 책임을 라우트별로 구분해 테스트
...
- `functions/company/[slug].js`: 32개 보험사 페이지의 동일한 고정 3개 관련글을 청구·실손·약관·수술/질병코드 5개 카테고리의 동적 5개 링크로 교체하고, 같은 생명/손해보험 type의 보험사 전산 링크 6개 추가.
- `functions/ga/[slug].js`: 17개 GA 페이지의 고정 관련글과 301 대상 `/insurance/practice/fp-income-structure` 링크를 제거하고, 설계사실무·채용팁·인수심사 카테고리의 동적 5개 링크 및 다른 GA 전산 링크 6개로 교체.
- `tests/seo-internal-links.test.js`(신규): 해시·offset/stride 결정성, SQL 바인딩, stride 순환 보충, 실패 시 빈 결과, HTML 이스케이프, 자기 제외·중복 제거, company/GA route 연결과 301 링크 제거를 회귀 테스트로 고정.

## [2.124.0] - 2026-07-12
### Added (공고 등록 모달 — 선착순 무료 프로모 히어로 배너)
- `index.html`: 공고 등록 모달(`#submit-modal`) 1단계 최상단(7~8월 등록비 인하 안내 배너 위)에 **선착순 100명 무료 공고 업로드** 광고 배너(`.sm-free-banner`) 추가 — 파랑→보라→분홍 그라디언트 히어로, "🔥 선착순 100명 무료" 배지(펄스 점)·"지금 등록하면 공고 업로드 무료!" 헤드라인·"등록비 20,000원 → 0원 · 남은 N자리" 서브카피. 기존 런치 프로모(`LAUNCH_PROMO` 선착순 100건 등록비 0원)와 동일 데이터를 사용.
- `smUpdatePrice()`: 배너 표시/숨김을 `smPromoActive()`(=`/api/promo/status`의 `remaining>0`)에 연동 — 프로모 잔여분이 있을 때만 노출하고, 남은 자리 수(`#sm-free-left`)를 `window.SM_PROMO.remaining`으로 실시간 반영. 소진(잔여 0)·비활성 시 자동 숨김으로 "무료"가 오안내되지 않도록 처리.
- 라이트/다크 테마 모두 흰 텍스트 그라디언트로 대비 확보, `prefers-reduced-motion`에서 광택·펄스 애니메이션 비활성. (헤드리스 크로미엄으로 노출·잔여반영·소진숨김·JS 오류 0 검증.)
...
- `index.html`: 모바일 채용·강의·모임 직접등록 CTA에 `7~8월 등록비 인하 이벤트` 라벨을 표시.

## [2.108.4] - 2026-07-06
### Added (관리자 승인요청 상세 확인·엑셀 내보내기)
- `admin.html`: 사용자 신청 탭의 채용·강의·모임 승인요청 카드를 클릭하면 신청자/공고내용/첨부파일/신청폼/등록비/할인권/추가옵션/동의기록을 팝업으로 확인할 수 있도록 추가.
- `admin.html`: 현재 승인대기 요청건을 UTF-8 CSV로 다운로드하는 `엑셀 내보내기` 버튼 추가.

## [2.108.3] - 2026-07-06
### Added (공고 직접등록 팝업 이벤트 배너)
- `index.html`: 내 공고 직접등록 팝업 상단에 `7~8월 등록비용 인하 이벤트 중` 배너를 추가하고, 채용·강의·모임 기본 등록비 20,000원 적용 문구를 노출.

## [2.108.2] - 2026-07-06
### Changed (공고 등록 옵션 가격·순서 조정)
- `functions/_lib/options.js`/`index.html`: `홈 배너 노출 7일` 가격을 300,000원으로 수정.
- `index.html`: 홈 배너 옵션의 `노출/클릭 리포트 제공` 문구를 제거하고, 추가 옵션을 낮은 가격부터 높은 가격 순서로 재배치.
- `index.html`: 현재 사용하지 않는 온라인 결제 안내/완료 화면 결제 박스를 사용자 화면에서 제거.

## [2.108.1] - 2026-07-06
### Changed (공고 등록 CTA·기본가·옵션 복구)
- `index.html`: 홈/사이드바 공고 등록 CTA 강조 스타일을 복구하고, 공고 등록 결제 모달의 채용·강의·모임 기본 등록비를 모두 20,000원으로 조정.
- `functions/_lib/coupons.js`: 서버 권위 기본 등록비(`AD_BASE`)를 채용·강의·모임 모두 20,000원으로 조정.
- `functions/_lib/options.js`/`admin.html`: 공고 등록 옵션에 `1:1 문의 기능` 20,000원, `추천공고 등록` 18,000원 표시/주문 라벨을 복구.

## [2.108.0] - 2026-07-06
### Added (공고 등록 옵션 — 오픈채팅방 홍보 일수형 과금)
- `functions/_lib/options.js`: `open_chat_promo` 옵션 추가 — **오픈채팅방 홍보 하루당 280,000원**. 클라이언트는 `{ key:'open_chat_promo', days:N }`만 전송하고 서버가 `280,000원 × 일수`로 최종 옵션가를 재계산한다. 일수는 1~30일 범위로 검증.
- `index.html`: 공고 등록 결제 스텝의 추가 옵션 영역에 **오픈채팅방 홍보** 체크박스와 일수 입력 추가. 선택 일수에 따라 추가 옵션 금액과 최종 결제액이 즉시 갱신된다.
- `admin.html`: 주문/환불 관리 표와 TSV 내보내기에서 `오픈채팅방 홍보 N일` 배지/옵션명으로 표시되도록 반영.
...
- `admin.html`: 지급 폼의 `#ID` 숫자 입력을 **닉네임/#ID 검색창 + 실시간 후보 드롭다운**으로 교체. 후보에 `#ID·닉네임·등급·차단여부` 표시, 클릭 선택 시 「선택됨 → #ID «닉네임»」 확인 표시. 300ms 디바운스, 응답 경쟁 시퀀스 가드, 바깥 클릭 시 닫힘, 검색어 수정 시 선택 자동 해제(오지급 방지). 선택 회원은 `_adminMemberNick`에 반영돼 지급 확인창에도 닉네임 노출. 지급 성공 시 폼 초기화.
- 서버 무수정 — v2.103.0에서 추가한 `/api/admin/members?q=` 닉네임/#ID 검색을 재사용.
### Verified
- admin.html 3블록 `node --check` 0 · 보안스캔 HIGH 0

## [2.103.0] - 2026-07-02
### Added (관리자 회원 목록 — 닉네임/#ID 검색창)
- **통합 기록지표 → 회원/지표 「회원 목록」에 검색창 추가**. 오픈채팅방순위 상위 회원을 이름으로 바로 찾을 수 있음(기존엔 페이지를 넘겨가며 눈으로 찾아야 했음).
- `functions/api/admin/members.js`: GET에 `q` 파라미터 추가 — 닉네임 부분일치(`LIKE`, `%`·`_`·`\` ESCAPE 처리) 검색. `q`가 숫자(또는 `#123`)면 회원 **#ID 정확일치**도 함께 매칭. SELECT·COUNT에 동일 WHERE 적용해 페이지네이션·결과수 정확. WHERE는 정적 문자열이고 입력은 전부 파라미터 바인딩 → 인젝션 안전.
- `admin.html`: 회원 목록 위에 검색 입력창(🔎 닉네임/#ID) + 검색·초기화 버튼. 입력 350ms 디바운스 + Enter/버튼 즉시 검색, 검색 결과 수 표시, 결과 없음 안내. 검색 상태는 20초 자동 갱신에도 유지.
### Verified
- `members.js` `node --check` 0 · admin.html 3블록 0 · 보안스캔 HIGH 0(members.js:57은 정적 WHERE 보간 오탐)
...
- `chat-popup.html` (신설, 정적 페이지): 자체 폴링(2.5초)·전송·렌더를 가진 경량 단독 채팅 페이지. 기존 `/api/chat` 그대로 사용(같은 도메인이라 로그인 쿠키 공유). 봇 메시지(member_id=0)·URL 자동 링크·줄바꿈 렌더, 메인 앱과 동일 테마(`ic-theme`)·알림음 설정(`ic-chat-sound`) 공유. 새 메시지 시 알림음 + (창이 가려져 있으면) 제목에 안 읽음 수 표시. 비로그인 시 카카오 로그인 게이트.
- `index.html`: 채팅 페이지 헤더와 사이드바에 **「⧉ 팝업으로 열기 / 실시간채팅 팝업」** 버튼 추가 — `window.open('/chat-popup.html', ...)` 로 420×640 팝업 오픈(팝업 차단 시 안내).
- 추가형 — 기존 채팅 로직·API 무수정. 팝업은 별도 정적 파일로 메인 번들에 영향 없음.
### Verified
- `chat-popup.html` 인라인 `<script>` 1블록 `node --check` 0 · index.html 인라인 29블록 0 · 보안스캔 HIGH 0

## [2.100.0] - 2026-06-29
### Added (실시간채팅 자동응답 봇 — 카카오 '삼따챗' 키워드 응답 포팅)
- **카카오 오픈채팅에서 쓰던 키워드 자동응답 봇을 라운지(실시간채팅)에 이식**. 라운지에 키워드를 입력하면 「삼따봇」이 같은 채팅에 정형 답변을 올림(전체 공개 — 모든 접속자에게 표시).
- `functions/_lib/chatbot.js` 신설: `getBotReply(text)` — 단어 그대로 매칭(카톡과 동일), 보험사명/약칭 정규화 조회, 긴 응답은 1500자 단위 청크 배열 반환.
  - 키워드: 5년납·7년납, 청구서류, 설계요청, 근재보험, 거주주택화재보험, 임대주택(부분), 휴게일반음식점, 요양병원·요양원·병원, 질병코드·약관·전산·뉴스·진료기록·우체국·할증요인, 간편보험·표준체, 자동차보험·펫보험, 소식지, 삼따(오픈채팅 유입), 키워드(도움말), 치아보험·간편치아보험, 카드납, 통합재가서비스, 보험사 연락처 36개사(약칭 포함).
- `functions/api/chat/index.js`: 라운지 POST에서 본문 매칭 시 봇 응답을 `member_id=0`(봇 sentinel)·닉네임 「삼따봇」으로 삽입(`DB.batch`, 비차단 `waitUntil`, 안전상 최대 60청크). 사용자 메시지 뒤(=더 큰 id)로 들어가 순서대로 표시.
- `index.html`: 봇 말풍선 스타일(🤖 아바타·청록 배경), 채팅 본문 **URL 자동 링크화**(`_chatLinkify`) — 라운지·1:1문의 공통. (`.chat-body`는 이미 `white-space:pre-wrap`이라 줄바꿈 유지)
- 추가형 — 기존 채팅 송수신 무수정. 봇은 사용자 POST에만 반응(봇 메시지는 DB 직삽입이라 재귀 트리거 없음).
### Verified
- `functions/_lib/chatbot.js`·`functions/api/chat/index.js` `node --check` 0 · getBotReply 매칭 테스트(약칭/키워드/청크) 통과(통합재가 7청크·최대 1493자) · index.html 인라인 29블록 0 · 보안스캔 HIGH 0

## [2.99.1] - 2026-06-29
### Changed (라운지 알림 대상 확대 — "전산창을 열고 있는 모두"에게 / 홍보·유입)
- **배경**: 실시간 채팅 라운지 알림이 푸시 구독자에게만 가던 것을, **앱 창을 열고 있는 모든 방문자(비로그인 포함)** 에게 포그라운드 알림(큰 소리+토스트)이 가도록 확대 — 채팅이 살아있음을 노출해 홍보·유입 강화. (1:1 문의는 사적 정보라 로그인 수신자 본인에게만 유지)
- `functions/api/chat/notify.js`: **비로그인 허용**으로 전환. 라운지 마지막 메시지는 공개로 항상 반환(+`created_at`), 1:1 문의는 로그인 시 본인 수신분만. 더 이상 401 없음.
- `index.html` 폴러: 비로그인 방문자도 폴링 지속(전원 대상). **진입 시 '지금 실시간 채팅 중!' 1회 유입 토스트**(직전 10분 내 대화면, 소리 없이) 추가. 활성 대화 시 도배 방지를 위해 라운지 **알림음은 최대 60초 1회로 스로틀**(토스트 동반). 라운지 페이지를 보고 있으면 억제.
- 참고: 백그라운드(창 내림/종료) 알림은 웹 표준상 **푸시 구독자**에게만 가능(미구독 브라우저엔 푸시 자체가 불가). "전원" 확대는 창이 떠 있는 포그라운드 경로에 적용.
### Verified
- `functions/api/chat/notify.js` `node --check` 0 · index.html 인라인 `<script>` 29블록 0 · 보안스캔 HIGH 0
...
- 답변 영역 높이를 **320px로 고정**하고 내용이 넘치면 **내부 스크롤**(overflow-y:auto)로 처리 → 근거사례를 펼쳐도 섹션·행 높이 불변, 옆 칼럼 영향 없음.
### Verified
- index.html 인라인 `<script>` 29블록 `node --check` 0 errors · 보안스캔 HIGH 0

## [2.98.3] - 2026-06-23
### Changed (삼따AI 답변 위치 + 자유게시판 인기글 강조)
- **삼따AI 답변을 질문창 바로 아래로 이동**: 기존엔 답변(out)이 예시칩·CTA 아래(섹션 하단)에 배치돼 입력창과 답변 사이 여백이 크게 남던 문제. DOM 순서를 `폼 → 답변 → 예시칩 → CTA`로 바꾸고, 답변 표시 시(`.aii-answered`) 답변이 빈 공간을 flex로 채우도록(예시칩 하단고정 해제) → 입력창 하단 여백 제거.
- **자유게시판 인기글 상단 강조**: 노출 글 중 가장 인기글(조회수 + 댓글×3 가중)을 맨 위로 올리고 `🔥 인기` 배지 + 앰버 강조 배경 적용.
### Verified
- index.html 인라인 `<script>` 29블록 `node --check` 0 errors · 보안스캔 HIGH 0

## [2.98.2] - 2026-06-23
### Fixed (자유게시판 하단 여백 — 글 수 7 → 15로 확대)
- 자유게시판 카드가 우측 대화순위(10위) 높이에 맞춰 늘어나며 글 7개로는 하단 여백이 크게 남던 문제. 노출 글 **7 → 15개**로 확대해 `.ibh-list`(flex:1 + 내부 스크롤)가 늘어난 높이를 꽉 채우도록 함. 넘치면 대화순위 패널처럼 내부 스크롤, 바로가기 푸터는 하단 고정. (board API는 page당 20개 반환이라 확보 가능)
### Verified
- index.html 인라인 `<script>` 29블록 `node --check` 0 errors · 보안스캔 HIGH 0

## [2.98.1] - 2026-06-23
### Fixed (홈 대시보드 후속 — 뉴스 2행 채움 + 자유게시판 글 수 확대)
- **인슈어커넥트 뉴스 2행 빈칸 제거**: 5열 레이아웃에서 노출 개수 8 → **10개**(5열 × 2행)로 늘려 2행의 4·5열 빈칸을 채움.
- **자유게시판 글 수 확대**: 대화순위 높이에 맞춰 늘어난 카드 공간을 채우도록 게시글 3 → **7개** 노출. `.ibh-list`를 `flex:1`+내부 스크롤로 전환해 남는 높이를 채우고 넘치면 스크롤(대화순위 패널과 동일 패턴, 행 높이 왜곡 방지).
### Verified
- index.html 인라인 `<script>` 29블록 `node --check` 0 errors · 보안스캔 HIGH 0
...
- **노출 스케줄**: 캠페인별 게시 시작/종료일(`start_at`·`end_at`, KST). 공개 GET은 **노출중(활성+스케줄내+이미지보유) 캠페인만** 반환 → 예약·자동 on/off. 관리자 편집은 `?all=1`(초안·예약·비활성 포함) 로드.
- **팝업 제어**: 자동 팝업 빈도(**하루1회/세션당/매번/끄기**)·지연(ms). 홈은 localStorage/sessionStorage 게이트로 과다 노출 방지(이탈 방지).
- **성과 대시보드**: 노출(배너 `imp`+팝업 `popimp`)·클릭(배너 `click`+CTA `cta`)·**CTR**을 캠페인별 집계(`?stats=1`, 7/14/30일 선택). 기존 `trackCardClick('홈광고',…)` 인프라 재활용 + `imp:`/`click:`/`cta:`/`popimp:`+campaignId 이벤트 신설. 레거시 `배너클릭`/`상담클릭`은 `main` 캠페인 귀속. (철수한 「파트너사 통계」의 성과측정 역할을 광고배너로 흡수)
- 관리자 「🖼 홈 광고배너」 패널: 캠페인 CRUD(추가/삭제·이미지 업로드/순서변경)·전역 로테이션/팝업 설정·캠페인별 성과 표.
### Note
- 홈 광고는 v2.89.0 이후 **자동 팝업 단독 노출**(인라인 배너 `.ibh-ad` 제거됨). 렌더러는 배너 재추가 대비 null-guard 유지 — 배너 부활 시 `imp`/`click` 자동 집계.
### Verified
- index/admin 인라인 `<script>` 문법 **0 errors**(JSON-LD 제외, `new Function`) · `home-ad/index.js` `node --check` OK · 옛 단일배너 식별자(`배너클릭`·`상담클릭`·`ha-enabled`·`_haImages`) 잔존 **0** · 보안스캔 HIGH 0 · release.mjs
...
- 광고는 기존 **「홈 진입 자동 팝업」(전체 이미지·크롭 없음)**으로 계속 노출 → 광고 노출 손실 없음. 모바일용 세로 이미지의 PC 크롭 문제도 해소. (로더는 `.ibh-ad` null 가드 있어 팝업·모달 정상 동작)
### Verified
- 보안 HIGH 0(REVIEW 51 기존) · release.mjs

## [2.88.0] - 2026-06-18
### Changed (홈 삼따AI 행 높이 축소 — 스크롤 부담↓)
- `ibh-aii-row`가 세로로 너무 길어 아래 섹션까지 스크롤이 길던 문제 → 우측 레일 콘텐츠를 컴팩트화해 행 높이 약 25%↓ (≈628px → ≈470px):
  - 자유게시판 미리보기 **5 → 3줄**
  - 포인트받기 「추천 순위」(리더보드) **숨김** — 친구초대·포인트 CTA는 유지 (사례기여순위처럼 정리)
  - 광고 `min-height` 130→90, 삼따AI `min-height` 340→260, 패딩·레일 간격 축소
### Verified
- 보안 HIGH 0(REVIEW 51 기존) · release.mjs
...
- `ibh-aii-row`를 3열(자유게시판+포인트받기 | 삼따AI | 광고)에서 **2열 「삼따AI 히어로(좌) + 자유게시판·포인트받기·광고배너 우측 레일」**로 재구성(`grid-template-areas: ai rail`). 삼따AI를 넓은 히어로로 강조하고, 자유게시판·포인트받기·광고배너를 같은 폭의 우측 레일에 세로 정렬해 사이즈 매치. 광고배너는 레일 하단을 채워(`flex:1`) 히어로와 바닥 정렬. `≤1024px` 1열 스택.
### Verified
- `home-clb` 잔존참조 0 · `grid-area` ai/rail 각 1 · `<button>` 310/310·`<section>` 10/10·`<div>` 1102/1102 균형 · 보안 HIGH 0(REVIEW 51 기존) · release.mjs

## [2.85.0] - 2026-06-18
### Changed (홈 공고 섹션 위치 이동)
- 채용·강의·모임 공고 3개 섹션을 「실시간 방문기록 보드」(상단 LIVE 방문자 카운터) **바로 아래로 이동**. 기존엔 뉴스·대화순위 행(`.ic-top-row`) 하단(전체너비 2행)에 있던 것을 페이지 상단(자유게시판·삼따AI 위)으로 끌어올려 공고 노출 강화. `.ic-top-row`는 뉴스 | 대화순위 2열만 유지.
### Added (공고 카드 조회수 표시)
- 채용·강의·모임 공고 카드에 **조회수(👁 아이콘 + 숫자)** 추가. 조회수 = 카드 열람 시 적재되는 `{type}_view` 이벤트 집계(`ic_link_clicks_daily`, v2.13.2부터 추적 중). 3개 목록 API(`recruitments`·`lectures`·`meetings/index.js`)에 `views` 서브쿼리 추가 → 홈 카드 렌더(`_viewsMeta`)가 날짜 옆에 노출. 모임 카드는 `조회수 · 👥 참여수` 병기.
### Verified
- 3개 API `node --check` OK · jobs-pair 1개(이동, 중복X) · `_viewsMeta` 4(정의1+사용3) · 섹션순서 방문strip<jobs<자유게시판<top-row · `<button>` 310/310·`<section>` 10/10·`<div>` 1106/1106 균형 · 보안 HIGH 0(REVIEW 51 기존) · release.mjs

## [2.84.0] - 2026-06-18
### Changed (로그인정보 액션 버튼 2열 그리드)
- 로그인정보 섹션의 4개 버튼(실시간 채팅·내 고객·만기관리·보장 분석·리모델링·내 공고 직접 등록)을 세로 스택 → **2×2 그리드(2열)**로 재배치. 세로로 길던 카드가 컴팩트해짐.
- 디자인 통일: 4개 버튼 모두 동일 크기·모양의 `.menu-pill` 셀로 매치(`min-height:50px`, 2줄 라벨 `word-break:keep-all`). 「내 공고 직접 등록」은 핵심 CTA라 앰버 액센트(`.is-cta`)만 부여해 같은 그리드 안에서 강조. 빠른 검색은 그리드 아래 전체너비 유지.
- 「내 공고 직접 등록」을 기존 아이콘+2줄(📤 / 💼🎓 채용·강의) 구조 → 그리드 셀에 맞는 단일 라벨로 단순화(동작 `openSubmitModal` 동일).
### Verified
- index.html parse: `<button>` 310/310 · `<div>` 1106/1106 균형 · `side-submit-single` HTML 참조 0(완전 대체) · 보안 HIGH 0(REVIEW 51 기존) · release.mjs

## [2.83.0] - 2026-06-18
### Changed (사이드바 빠른메뉴·로그인정보 재구성)
- 빠른 메뉴: 「사례공유 +P」 버튼 삭제. 「내 고객·만기관리」·「보장 분석·리모델링」 버튼을 **로그인정보 섹션으로 이동** → 빠른메뉴는 뉴스 + 전산/GA/수납/소식지/청구/채용/강의/모임/실무도구/인수Q&A(서비스·도구)만 남아 깔끔하게 정리.
- 로그인정보 섹션: 실시간 채팅·내 고객·보장 분석 3개 버튼을 **무채색 `.menu-pill` 디자인으로 통일**(기존 파란 채팅 버튼 그라데이션·실시간 점 제거 → 나머지 버튼과 동일). 개인 도구(채팅·고객·보장분석)를 로그인정보 영역에 그룹화.
- 「MANAGER LINK(설계매니저 커뮤니티)」 버튼 삭제 — 사이드바 커뮤니티 섹션·모바일 메뉴·온보딩 투어 문구에서 모두 제거. CREW 300은 유지.
- 모바일 동일 적용: 로그인정보 카드 실시간 채팅 버튼도 무채색 `.menu-pill`로, MANAGER LINK 카드 제거.
- 정리: 미사용 `.side-chat-btn` CSS·`scbPulse` 키프레임 제거.
### Verified
- index.html parse: `<button>` 310/310 균형 · `side-chat-btn` 잔존참조 0 · MANAGER LINK 버튼 라벨 0 · CREW 300 유지 · `<a>` 델타 불변(기존값) · 보안 HIGH 0(REVIEW 51 기존) · release.mjs

## [2.82.0] - 2026-06-18
### Changed (홈 대시보드·사이드바 디자인 정리 — 사용 편의 중심)
- 삭제: 홈 「📰 종합 뉴스」(외부 RSS)·「🎬 경제·뉴스 영상」(유튜브) 보드 2종 제거. 자체 콘텐츠(인슈어커넥트 뉴스·진행중 채용/강의/모임·오픈채팅 대화순위·삼따AI·자유게시판) 중심으로 정리 → 외부 스크랩 콘텐츠 제거로 AdSense thin-content 우려도 완화. 자체 「인슈어커넥트 뉴스(카드뉴스)」는 유지.
- 사이드바 빠른 메뉴: 버튼 이모지 전부 제거(📰💬👥🧮🩺💡)·「⚡」 헤더 이모지 제거. 「실시간 채팅」 버튼은 빠른메뉴 → **로그인정보 섹션**(로그인 버튼 바로 아래)으로 이동 + 실시간 표시 점으로 강조.
- 홈 섹션 제목 이모지 톤다운: 자유게시판·삼따AI·인슈어커넥트 뉴스·진행중 채용/강의/모임·오픈채팅방 대화순위 (온보딩 투어 툴팁 이모지는 유지).
- 색감 통일: 자유게시판 카드 amber → 브랜드 블루 계열로 전환(배지·링크·hover 포함), 자유게시판·삼따AI 카드 테두리를 `var(--border)`로 통일 + `var(--sh-sm)` 그림자 일관화 → 인기 콘텐츠·대화순위 카드와 한 톤.
- 모바일 동일 적용: 빠른 메뉴 카드 이모지 제거(💬👥🧮🩺·❓사용법)·「실시간 채팅」 카드 → 로그인정보 카드(`.m-account-card`)로 이동.
### Verified
- index.html parse: `<button>` 311/311 · `<section>` 10/10 균형 · 삭제 보드 잔존참조(ibh-news/ibh-vids/vw-grid/nw-grid) 0 · 보안 HIGH 0(REVIEW 51 기존) · release.mjs
...
- 기존엔 차단돼도 **재로그인이 됐고 글쓰기만 막혀** 사유 안내가 없었음 → 이제 로그인 단계에서 명확히 안내 + 문의 경로 제공. `functions/api/auth/kakao/callback.js`에 `ic_banned_members` 게이트 추가, 문의 링크는 사이트 연락처 오픈채팅(`open.kakao.com/o/sAZWQ7pi`)과 동일.
### Verified
- callback.js parse · 차단게이트 마커 · 보안 HIGH 0 · release.mjs

## [2.79.0] - 2026-06-14
### Fixed (할인권 무상지급이 엉뚱한 회원에게 가던 치명 버그)
- **원인**: 지급 폼은 회원 **DB ID**가 필요한데, 「회원 관리」 목록은 **순번(seq)만 크게** 표시하고 실제 DB ID는 툴팁에만 숨어 있었음. 회원 삭제로 ID 공백이 많아(현재 255명·최대ID 385 → **순번≠DB ID**) 관리자가 순번을 ID로 입력 → 다른/없는 회원에게 적재되고 의도한 회원은 못 받음. (프로덕션 D1 직접 조회로 확정 — 유일 지급분이 #45에 적재, 데이터·조회경로 자체는 정상)
- **수정**: ① 회원 목록 1열에 **실제 #DB ID를 파란색으로 크게** 표시(순번은 작게 병기)·헤더 「회원ID」. ② 지급 **확인창에 대상 닉네임 «닉네임» 표시** — 순번 혼동 시 즉시 인지. ③ 할인권 종류 미선택 가드 + 안내문구 「파란 #ID 입력」으로 정정.
- `user_coupons`/`/api/coupons`/me.js 데이터·표시 경로는 정상 — **입력 UX**만 문제였음.
### Verified
- admin 3블록·index 33블록 0오류 · 프로덕션 D1 조회로 근본원인 확정 · 보안 HIGH 0 · release.mjs

## [2.78.0] - 2026-06-14
### Added (관리자 — 상위 2뷰 좌우 슬라이드 + 회원 목록 페이지당 인원 선택)
- **「📊 통합 기록지표」 ↔ 「📦 콘텐츠 업로드 & 관리」 좌우 슬라이드 전환** — 두 상위 섹션이 세로로 길게 쌓여 있던 것을 좌우로 번갈아 보도록 변경. 상단 세그먼트 버튼 + ‹ › 화살표 + 터치 스와이프(탭바·표·내부 슬라이더·입력은 제외해 스크롤 충돌 방지) + 슬라이드인 애니메이션. 기본 진입은 통합 기록지표. `overflow-x:clip`으로 sticky 업로드 탭 영향 없음(`hidden` 미사용).
- **회원 목록 페이지당 인원 선택(10/20/50/100명씩)** — 회원이 많아져 관리가 힘든 문제 해결. 선택값은 localStorage 기억(기본 20명). 서버 `/api/admin/members?per=`(5~200 캡) 반영, 순번·총 페이지수 자동 재계산.
### Verified
- admin 3블록·index 33블록 0오류 · `members.js` parse · `.av-slide`(2)·`#av-uploads` 개폐 균형·`adminView`/`setMemberPer`/`mt-per` 배선 그렙 · 보안 HIGH 0 · release.mjs

## [2.77.0] - 2026-06-14
### Changed (환불 정책 「전문 보기」 — alert → 정식 스크롤 모달)
- 등록 모달의 「정책 전문 보기」가 `alert` 창이라 길고 모바일 가독성이 나빴던 것 → **스크롤 가능한 정식 모달**(`#refund-policy-modal`)로 업그레이드. 기존 `.sm-overlay/.sm-modal` 디자인 재사용(헤더·✕·스크롤·다크모드 자동 대응), 등록 모달 위 레이어(z-index 100006), **배경 클릭·✕·확인 버튼**으로 닫기. 내용은 ①게시 전 / ②게시 후 / ③운영자 귀책 + 공통을 **색상 구분 섹션**으로 구조화(정책 내용 자체는 v2.76.0과 동일).
### Verified
- index 33블록 0오류 · 모달 마크업·show/close 배선·링크 호출 확인 · 구 alert 텍스트 제거 · 보안 HIGH 0 · release.mjs

## [2.76.0] - 2026-06-14
### Changed (공고 등록 환불 정책 — 모순 제거 + 운영자 기준 정비)
- 기존 「② 게시 후·공개 전 90% 환불」은 이 플랫폼에서 **승인=게시=공개가 동시**라 존재할 수 없는 구간이었음 → 모순 제거. 정책을 **게시 전 / 게시 후 2단계**로 명확화:
  - **게시 전**(승인·노출 시작 전): 전액 환불 + 사용 할인권 1회 복원
  - **게시 후**(노출 개시 이후): 환불 불가 — 게재기간·잔여일·조회수·지원/참여/문의 실적과 무관, 상단노출 등 부가옵션도 적용 후 불가
  - **운영자 귀책**: 전액 환불 또는 동등 재게재
- 운영자 보호 강화: 환불 신청 **결제일 7일 이내·게시 전만**, 포인트·할인권 환불/현금화/양도 불가(게시 후 미복구), 성과 미달은 환불 사유 아님, 허위·다계정·악용 시 환불 없이 게시중단. **강행규정 우선** 조항으로 약관 무효 방지.
- 동의 체크 3종·`smShowRefundPolicy` 전문·`orders.js` 주석·관리자 환불 UI(90%→「부분(재량)」) 일괄 정합화. 기존 ①(할인권 원복)과 동의2(미복구) 불일치도 해소.
### Verified
- index 33블록·admin 3블록 0오류 · `orders.js` parse · 구 문구 잔존 0 · 보안 HIGH 0 · release.mjs
...
- **1초 폴링 타이머 제거**: 모바일 테마 active 동기화(`_mApplyThemeActive`)가 매초 `localStorage` 읽기+`querySelectorAll`로 폴링하던 것 → `applyTheme()`(테마 변경 시점)에서 모바일 버튼도 함께 토글하는 **이벤트 구동**으로 전환. 상시 CPU 깨움 제거.
- **시계 1초 틱 가드**: 데스크톱/모바일 시계가 각자 반대 레이아웃(숨김 상태)에서도 매초 DOM을 갱신하던 낭비 제거 → 자기 레이아웃 활성 + 탭 `visible`일 때만 틱.
- **60초 폴러 visibility 가드**: 모바일 통계 폴러가 탭 비활성 시에도 `/api/stats`를 치던 것 → `visible`일 때만.
### Verified
- index 33블록 0오류 · `_headers` `/`·`/index.html` 양쪽 규칙 · 보안 HIGH 0 · release.mjs

## [2.73.0] - 2026-06-13
### Changed (홈 「오픈채팅방 대화순위」 여백 제거 — 뉴스 높이에 맞춤)
- 대화순위 패널이 좌측 컬럼 **전체(뉴스+진행중 공고)** 높이에 맞춰 늘어나 10개 항목이 넓게 퍼지며 여백이 컸던 문제 수정. 레이아웃을 **[📰 인슈어커넥트 뉴스 | 💬 대화순위] 1행(높이 일치) + [진행중 채용/강의/모임] 전체너비 2행**으로 재구성(`grid-template-areas`, `.ic-top-left` 래퍼 제거). 대화순위가 **뉴스 섹션 높이에 딱 맞아** 여백 제거. ≤1024px 세로 스택.
### Verified
- index 33블록 0오류 · `.ic-top-left` 언랩·`grid-area`(news/jobs/kakao) 배선 그렙 일치 · 보안 HIGH 0 · release.mjs

## [2.72.0] - 2026-06-13
### Fixed (모바일에서 광고 '팝업'이 안 뜨던 문제 + 인라인 배너 롤백)
- **모바일에서 광고 자동 팝업이 안 뜨던 버그 수정** — 팝업 모달(`#home-ad-modal`)이 `#page-home`(모바일에서 `display:none`) 안에 있어 갇혀 있었음(부모가 display:none이면 position:fixed 자식도 숨겨짐). 모달을 **`document.body`로 reparent** 해 데스크탑·모바일 모두 정상 노출(홈 진입 시 자동 팝업, 페이지 로드당 1회).
- 직전 추가한 모바일 인라인 배너(`.ibh-ad-m`)는 **롤백**(요청은 '팝업'이었음). 데스크탑 인라인 배너(`.ibh-ad`)는 기존대로 유지. 모바일 「고객 렌트카·통신 견적 연결」 삭제는 유지.
### Verified
- index 33블록 0오류 · `ibh-ad-m` 제거(0) · 모달 body 이동 배선 · 보안 HIGH 0 · release.mjs

## [2.71.0] - 2026-06-13
### Changed (모바일 — 광고배너 노출 + 견적연결 삭제)
- **모바일 홈에도 광고배너 노출** — 관리자 업로드 배너(`/api/home-ad`, 데스크탑과 동일 config)를 모바일 홈(`#page-home-mobile`)에 표시(`.ibh-ad-m`, 글로우+샤인 애니, 클릭 시 팝업·자동팝업). 기존엔 데스크탑만 노출됐음. *(관리자가 「🖼 홈 광고배너」에서 이미지 업로드+노출ON 해야 표시됨 — 현재 미설정이면 안 뜸)*
- **모바일 「🚗📱 고객 렌트카·통신 견적 연결」 CTA 삭제**(요청) — 그 자리에 위 광고배너 배치. (파트너사 페이지 자체는 유지)
### Verified
- index 33블록 0오류 · `m-quote-cta` 제거(0) · `ibh-ad-m` 배선 그렙 일치 · 보안 HIGH 0 · release.mjs

## [2.70.0] - 2026-06-13
### Changed (모임공고 참여 게이트 — 로그인+참여해야 상세 공개)
- **모임 상세(장소·일시·설명·신청폼·이미지·참여자 명단)를 로그인+참여 뒤로 잠금** — 제3자가 참여 버튼 없이 신청폼/장소를 보고 무단 참석하던 리스크 차단. 공개는 **제목·주최·참여수**뿐. **서버단 차단**(목록·상세·OG·SSR·명단 전부): 비참여자는 네트워크로도 상세를 못 봄. 참여 즉시 잠금 해제.
  - `GET /api/meetings`(공개)=제목·주최·참여수만(관리자=전체) · `GET /api/meetings/[id]`=관리자/주최자/참여자만 전체+명단, 그 외 `locked` teaser · `/api/meeting-participants` 명단은 참여자만.
  - OG `/og/meetup`·SSR `/meeting`·sitemap=제목·주최 teaser(noindex). 뷰어=🔒 게이트 → 「로그인하고 참여하기」 → 상세·명단 공개.
- 트레이드오프: 모임 상세는 검색 색인에서 빠짐(참여 게이트 정책). 채용·강의는 기존대로 공개 유지.
### Verified
- `node --check`(meetings·[id]·participants·og·meeting·sitemap) OK · index 33블록 0오류 · 게이트 배선 그렙 · mtpLoad 잔존 0 · 보안 HIGH 0 · release.mjs

## [2.69.0] - 2026-06-13
### Added (공고 등록 시 할인권 등록비 할인 안내)
- 채용·강의·모임 **공고 등록 모달에 할인권 할인 안내** 보강: ① 1단계 상단 배너 "등록비는 할인권으로 할인 — 채용 15%·강의 35%·모임 49% · 할인권 보기" ② 결제 단계 **공고타입별 동적 안내**(`#sm-coupon-hint`) — 보유 시 "N개 선택하면 자동 할인(최대 N%)", 미보유 시 "최대 N% 할인 가능, 포인트로 교환 → 할인권 교환 링크(/me)". 기능 변경 없이 인지 안내만 추가.
### Verified
- index 33블록 0오류 · `sm-coupon-hint`/`SM_MAX_RATE` 배선 그렙 일치 · 보안 HIGH 0 · release.mjs
...
- **주문 `ad_orders`**(등록가·할인권·최종가·상태·동의3종) 공고 등록 시 자동 생성(채용/강의/모임 POST, best-effort) + **환불 로그 `refund_logs`**(런타임 lazy, 기존 테이블 무수정).
- **관리자 「💰 환불/주문」 탭**: 주문목록(공고·신청자·가격·동의·공고상태) + 환불 **전액**(게시전·할인권 원복)/**90%**(게시후 공개전)/**거절**(사유 기록) + 「동의 기록 내보내기(TSV)」. 신규 `GET/POST /api/admin/refunds`.
- **승인요청 카드에 환불정책 동의 기록 표시**(공개후환불제한/포인트/모집실패 ✓✗). 기존 쿠폰·가격·상단노출·게시판·마이페이지 전부 유지, 새 포인트·직접결제 없음.
#### 보고
1. 변경파일: `_lib/orders.js`·`api/admin/refunds.js`(신규), `api/{recruitments,lectures,meetings}/index.js`·`index.html`(동의)·`admin.html`(환불탭·승인큐). 2. DB: 신규 `ad_orders`·`refund_logs`(lazy CREATE). 3. 정책위치: 동의=제출 모달, 환불=`/api/admin/refunds`(상태기반), 쿠폰원복=full 환불 시 user_coupons active 복원. 4. KPI: 등록전환·환불율·동의완결률·등록매출. 5. 위험: 수기입금이라 환불은 의사결정·기록(실반환 수기), 동의는 프론트 게이트+백엔드 기록. 6. 롤백: 환불탭·동의 제거 + createAdOrder 호출 제거(테이블 잔존 무해).
### Verified
- `node --check`(orders·refunds·3 posting POST) OK · admin 3·index 33블록 0오류 · 동의/환불/주문 배선 그렙 일치 · 보안 HIGH 0 · release.mjs

## [2.66.0] - 2026-06-13
### Fixed (공고 주최자가 본인 공고 1:1 문의에 답변 못하던 버그)
- 공고(채용·강의·모임) 주최자가 **본인 공고에서 「1:1 문의하기」를 누르면 "본인이 등록한 공고입니다"로 막혀** 들어온 문의에 답변할 수 없던 문제 수정. 이제 주최자가 본인 공고 채팅을 열면 **그 공고에 들어온 문의 목록(inbox)**이 떠서 문의자별로 답장 가능. (`/api/chat/dm`: ad_type+ad_id 요청자가 주인이면 `is_owner` + 해당 공고 문의방 목록 반환) 접근통제는 그대로(참여자만 열람).
### Changed (홈 광고배너 다중 이미지 — 순번 업로드)
- 홈 광고배너를 **여러 장(최대 20) 순번 업로드**로 확장(카드뉴스처럼 파일명 1,2,3…순 정렬). **배너=1번 이미지**, 홈 클릭/자동 팝업·SEO 팝업은 **전체를 순서대로** 표시. 관리자 「🖼 홈 광고배너」에서 다중 추가·↑↓ 순서변경·삭제·노출토글. `/api/home-ad` config가 `images[]` 배열(레거시 단일 폴백)·URL 검증.
### Verified
- `node --check`(dm·home-ad·seo-cta) OK · index 33·admin 3블록 0오류 · is_owner/다중이미지 배선 그렙 일치 · 보안 HIGH 0 · release.mjs
...
- **홈 진입 시 광고 팝업 자동 노출**(노출 ON + 팝업 이미지 설정 시 · 페이지 로드당 1회 · 홈에서만).
- **홈 「실시간 인기 콘텐츠」 → 「📰 인슈어커넥트 뉴스(최신순)」** 교체 — 카드뉴스 최신 세트 타일(클릭→뷰어)·전체보기 링크. (loadLiveMonitor 이름 유지, 내용만 뉴스로)
- **오픈채팅방 대화순위** 항목을 패널 높이에 맞춰 세로 균등 분산(`space-between`+항목 패딩) — 늘어난 섹션 길이에 내용 정렬.
### Verified
- `node --check`(home-ad·seo-cta) OK · index.html 인라인 33블록 0오류 · connect.webp 잔존 0 · 배선 그렙 일치 · 보안 HIGH 0 · release.mjs

## [2.63.0] - 2026-06-13
### Added (관리자 — 모임 관리 탭 · 1:1 문의 모니터 · 홈 광고배너 관리)
- **모임공고 전용 관리 탭(🤝 모임)** — 채용·강의와 동일하게 직접 등록(주최/장소/일시/설명/첨부 → `uploadMeeting`) + 등록 목록(노출·삭제). 기존 강의 탭에 얹혀 있던 모임 관리를 분리·승격.
- **1:1 공고 문의 채팅 모니터(💬 문의채팅 · 읽기 전용)** — 신규 `GET /api/admin/dm`(`?rooms=1` 목록 / `?room_id=` 메시지). 방 목록(공고·양측 닉·메시지수) → 전체 대화 열람. 운영 감독용(전송 불가).
- **홈 광고배너(삼따AI 옆) 관리(🖼 홈 광고배너)** — 하드코딩(`/connect.webp`·naver 링크)을 관리자에서 배너/팝업 이미지 업로드·CTA 링크·노출 토글로 교체. 신규 `/api/home-ad`(GET 공개=기본값 동일·무변경 / PUT 관리자, 범용 `ic_site_config`). 홈은 설정 fetch로 동적 적용(미설정 시 기존 화면 유지).
### Verified
- `node --check`(admin/dm·home-ad) OK · admin.html 3블록·index.html 33블록 0오류 · 탭/패널/함수/lazy-load 배선 그렙 일치 · URL 검증(http(s)/상대경로만) · 보안 HIGH 0 · release.mjs

## [2.62.0] - 2026-06-13
### Added / Changed (홈·사이드바 정리)
- **홈 대시보드에 「🤝 진행중인 모임」 섹션** 추가 — 진행중인 채용공고·강의 옆 **3열 배치**(`/api/meetings?limit=4`, 카드 클릭→모임 뷰어·공유). ≤1024px 세로 스택.
- **오픈채팅방 대화순위**를 상위 **10등까지만** 표시로 변경(기존 20).
- **사이드바 빠른메뉴 정리**: 🤖 삼따AI·🤝 파트너사 버튼 삭제, 나머지를 균형 2열로 정돈(💡 사례공유를 wide로 승격 → 일반 pill 10개=5행 정렬, 고아줄 제거).
### Verified
- index.html 인라인 33블록 0오류 · 모임 홈섹션/메뉴/순위 배선 그렙 일치 · 보안 HIGH 0 · release.mjs
...
- → **모임공고 운영 루프 완성**: 유저 등록(pending) → 관리자 승인 → 공개 게시 → 공유·상단노출·할인권. (백엔드 2.57.0 + 프론트 2.58.0 + 관리자 2.59.0)
### Verified
- admin.html 인라인 3블록 0오류 · index.html 33블록 0오류 · 승인큐/관리/featureAdmin 모임 배선 그렙 일치 · 보안 HIGH 0 · release.mjs

## [2.58.0] - 2026-06-13
### Fixed (카드뉴스 공개피드 데이터 누락 — 오래된 세트가 조금씩 사라지던 버그)
- **인슈어커넥트 뉴스(카드뉴스) 공개 피드에서 업로드가 쌓일수록 오래된 세트가 사라지던 버그 수정.** 원인: 공개 `GET /api/card-news` 가 **슬라이드** 기준 `LIMIT 200` — 카드뉴스는 **세트(여러 슬라이드)** 단위로 소비되는데, 누적 슬라이드가 200을 넘으면 가장 오래된 **세트**가 잘려 공개뷰에서 누락됨(DB·관리자(limit=300)에는 정상 → "관리자엔 멀쩡한데 공개에서만 조금씩 사라짐"). **데이터 유실이 아니라 조회 누락이었음.**
- 수정: 공개 피드를 **세트 단위**로 — 최신 N개 '세트'를 **모든 슬라이드와 함께** 반환(JOIN+GROUP BY, 세트가 절대 부분잘림/통째누락 되지 않음). 프로덕션 확인: 기존 공개뷰 21/24 세트 → 24/24 세트.
### Added (모임공고 #3b-B — 신규 공고타입 프론트엔드, 채용/강의와 동일 UX)
- **🤝 모임** 페이지(`#page-meetup`·빠른메뉴 pill·모바일 코어카드)·등록 모달 **3번째 모드**(주최/장소/일시 필드·등록가 149,000)·`?meeting={id}` 딥링크·공유(`/og/meetup/{id}`)·NEW 배지. 등록 시 **모임 할인권**(mt10~mt49) 자동 할인, `/me` 상점에 모임 할인권 공개. 업로드 폴더 화이트리스트에 `meetings` 추가.
- → 모임공고 = 채용·강의와 동일한 풀 루프(등록→승인→게시→공유→상단노출→할인권). 백엔드는 v2.57.0(증분3b-A).
### Verified
- `node --check`(card-news·meetings·me·user-upload) OK · index.html 인라인 33블록 0오류 · 모임 배선(모달/페이지/딥링크/메뉴/API) 그렙 일치 · 보안 HIGH 0 · release.mjs
...
- → **수익화 루프 완성**(채용·강의): 포인트 적립 → /me에서 할인권 교환 → 공고 등록 시 적용 → 입금액 할인. (모임공고는 net-new 공고타입 = 증분3, 관리자 쿠폰관리·통계도 증분3)
### Verified
- 인라인 33블록 0오류 · smLoadCoupons/smUpdatePrice/결제블록/payload coupon_id 배선 · 보안 HIGH 0 · release.mjs
...
- **기능 보존**: 카카오 로그인(`/api/auth/kakao/login`)·게이트 show/hide(`mlg-hidden`)·초대배너(`#mlg-invite`)·소셜프루프(`#mlg-social`)·SEO 콘텐츠 링크(`/insurance`·`/recruit`) 그대로. H1=「보험으로 연결하다」. 기존 `.mlg-*` CSS 유지(미사용분 무해).
### Verified
- 인라인 33블록 0오류 · 기능훅(gate·invite·social·kakao·H1) 보존 · 보안 HIGH 0 · release.mjs

## [2.51.0] - 2026-06-10
### Added (공고 수익화 #1 — 공고 할인권: 포인트→쿠폰, 추가형·무위험)
- **공고 등록 할인권(쿠폰) 상점**: 기존 포인트로 채용/강의 할인권을 교환. 마이페이지(/me)에 「🎟️ 공고 할인권」 카드 신설(상점 + 보유 할인권 D-day). 신규 포인트 생성 없음 · 직접 포인트 결제 없음(쿠폰 경유) · 유효기간 14일.
- 신규 `functions/_lib/coupons.js`(코드 고정 카탈로그·등록가·`final=등록가×(1-할인율)`·하한 없음) + `functions/api/coupons/index.js`(GET 보유+카탈로그+포인트 / POST 구매). 구매는 `redeem.js`와 **동일 차감 패턴**(`ic_members.points` 감소 + `ic_point_log` reason `shop_coupon_*` → 기존 상점 통계에 자동 집계). 테이블 `coupon_templates·user_coupons·coupon_logs`는 `referral.js`처럼 **런타임 lazy 생성**(기존 테이블 무수정).
- 카탈로그(스펙): 채용 10%=100P·15%=180P / 강의 10%=180P·25%=350P·35%=500P / 모임 10%=300P·20%=500P·30%=800P·49%=1500P. 상점 UI는 등록 흐름이 있는 채용·강의만 노출(모임은 net-new 공고타입이라 증분3).
- **기존 구조 무수정**(포인트·공고·상점·결제 흐름 그대로). 증분2=공고 작성에 할인 적용(등록가/절감/최종결제) · 증분3=관리자 쿠폰관리·통계 + 모임공고.
### Verified
- `node --check`(coupons lib·API·me.js) OK · 보안 HIGH 0 · release.mjs (배포 후 미인증 401 확인)

## [2.50.0] - 2026-06-10
### Changed (네비게이션 투어 — 신기능 안내 보강, 사용유도)
- 투어 quickmenu 스텝 설명에 **내 고객·보장분석·인수 Q&A** 추가 + **「👥 내 고객·만기·보장분석」 전용 스텝 신설**(`[data-tour='clients']` 앵커를 데스크톱 빠른메뉴 pill·모바일 코어카드에 추가, `icTourFind`가 보이는 쪽 자동 선택). 새로 만든 리텐션 기능을 신규 사용자가 첫 투어에서 발견하도록.
### Verified
- IC_TOUR 8스텝 · `data-tour=clients` 앵커 2 · 인라인 33블록 0오류 · 보안 HIGH 0 · release.mjs

## [2.49.0] - 2026-06-10
### Added (보장분석 ↔ 내 고객 연동 — 고객별 보장 저장·재조회로 lock-in 강화)
- **내 고객 카드 「🧮 보장분석」 버튼** → 그 고객 컨텍스트로 보장 분석 페이지 진입. 저장된 분석이 있으면 자동 불러와 즉시 재분석, 페이지 상단 **고객바**(👤 OOO 님의 보장 / 일반 모드 전환). **「💾 고객에 저장」**으로 분석을 그 고객에 저장(카드에 ✓ 표시). 고객을 다룰 때마다 보장 갭을 추적 → 갱신·리모델링 영업으로 직결.
- API `functions/api/me/clients.js`에 **`coverage` 컬럼 추가**(기존 테이블 `ALTER TABLE ... ADD COLUMN` 마이그레이션·중복 무시) + GET/POST/PATCH 반영(JSON 문자열, 본인 데이터만). 빠른메뉴/모바일 일반 진입은 고객 컨텍스트 초기화.
### Verified
- `node --check`(clients.js) OK · 인라인 33블록 0오류 · covEnter/openCoverageForClient/saveCoverageToClient·coverage 8건·CRM버튼·고객바 배선 · 보안 HIGH 0 · release.mjs

## [2.48.0] - 2026-06-10
### Added (보장 분석 · 리모델링 — 보류했던 증권분석의 OCR-free MVP, 영업 무기)
- 신규 **「🧮 보장 분석」**(`#page-coverage`, 빠른메뉴 wide·모바일 코어카드): 고객의 현재 보장(실손·암/뇌/심장 진단금·사망·입원일당·수술비·후유장해·운전자·일배책 10항목)을 입력하면 **권장 대비 충분/부족/공백 진단 + 충실도 점수 + 리모델링 우선순위**를 산출. 상담·리모델링 영업 무기. **클라이언트 전용(백엔드 0)**, 권장치 투명 표기.
- 안전: "일반 권장 기준 **참고용 도구**, 고객 상황별 상이, 최종 판단은 설계사" 면책 명시(개인 맞춤 자문 아님). 증권 OCR/약관 파싱은 별도 설계 과제로 후속.
- 빠른메뉴는 wide로 추가해 일반 pill 5쌍 짝수 정렬 유지(v2.47.0 정리 보존).
### Verified
- 인라인 33블록 0오류 · page/fn/메뉴 배선(coverage refs 2) · 보안 HIGH 0 · release.mjs

## [2.47.0] - 2026-06-10
### Changed (빠른메뉴 사이즈 정리 — 미관 통일)
- 빠른메뉴 pill이 wide(전체폭)·일반(2열) 혼재로 **고아 pill 3개(강의·사례공유·실무도구)+줄바꿈**이 생겨 들쭉날쭉하던 문제 → **순서 재배치**: wide는 위2(뉴스·내 고객)·아래2(삼따AI·파트너사)로 묶고, 일반 10개를 **5쌍 짝수 정렬**(전산·GA / 수납·소식지 / 청구·채용 / 강의·실무도구 / 인수Q&A·사례공유). 긴 라벨 단축(인수 Q&A). `.menu-pill`에 **min-height 42px + flex 중앙정렬**로 모든 pill 높이 균일화.
### Verified
- side-quickmenu pill 14(wide 4·일반 10=5쌍) · 인라인 33블록 0오류 · 보안 HIGH 0 · release.mjs
...
### Verified
- 인라인 33블록 0오류 · CTA/pill 배선 확인 · 보안 HIGH 0 · release.mjs

## [2.44.0] - 2026-06-10
### Added (리텐션 #4+#5 — 일간 모닝 브리핑 + 만기 푸시: 매일 여는 트리거)
- 신규 `POST /api/cron/daily-brief` — **회원별 개인화 일간 브리핑**: ⏰ 내 **만기·갱신 임박 고객**(미니 CRM `ic_client_notes`, 지난 3일~향후 7일) + 📢 플랫폼 새 소식(채용·강의·카드뉴스) → **카톡 메모 + 웹푸시**. "매일 여는 트리거"이자 "만기 알림으로 갱신 매출 사수"(리텐션 4·5 통합).
- **안전 게이트**: 기본 **DRY**(미발송·미리보기만). `DAILY_BRIEF_ENABLED='1'` + 인증(관리자/`CRON_SECRET`)일 때만 발송. 하루 1회 빈도가드, 보낼 내용 없는 회원은 발송 생략(스팸 방지), revoked/만료 구독 자동 비활성. 푸시는 `member_id` 매핑으로 개인화.
- **딥링크 `/?go=<page>`**: 푸시/카톡 클릭 시 해당 페이지 자동 오픈(브리핑→「내 고객」 바로가기).
- **활성화(운영 절차, 미실시)**: ① `wrangler pages secret put CRON_SECRET` ② 외부 cron 매일 1회 POST(Worker/GitHub Actions/cron-job.org) ③ DRY 미리보기 검증 ④ `DAILY_BRIEF_ENABLED=1`.
### Verified
- `node --check`(daily-brief) OK · 인라인 33블록 0오류 · 딥링크 핸들러 확인 · 보안 HIGH 0 · release.mjs (배포 후 미인증 401 확인)

## [2.43.0] - 2026-06-10
### Added (리텐션 #3 — 미니 CRM 「내 고객·만기 관리」: lock-in 결정타)
- 설계사 **본인에게만 보이는 비공개 고객 메모**(`#page-clients`, 빠른메뉴·모바일 코어카드 진입). 고객명·연락처·보험사·상품·월보험료·**만기/갱신일·메모** 저장. 만기 **D-day 배지** + 30일 내/지난 건 **상단 알림 배너**로 갱신 상담을 놓치지 않게.
- 신규 API `functions/api/me/clients.js` — 카카오 로그인 세션(`getUserFromRequest`) 인증, 모든 쿼리 `member_id` 일치 강제(타 회원 접근 차단). `GET/POST/PATCH/DELETE` CRUD, D1 `ic_client_notes` 지연 생성(인덱스 포함), 본인당 1000건 한도, 입력 길이 제한.
- **개인정보**: 설계사 본인의 업무 메모(고객 갱신/만기 관리). 주민번호 등 민감식별정보는 받지 않음. "본인 계정에만 보이는 비공개" 명시. → *자기 데이터가 여기 쌓여 매일·여러 번 열게 되는 lock-in.*
### Verified
- `node --check`(clients.js) OK · 인라인 33블록 0오류 · page/fn/메뉴/로드훅 배선 확인 · 보안 HIGH 0 · release.mjs (배포 후 미인증 401 + 페이지 게이트 확인)

## [2.42.0] - 2026-06-10
### Added (리텐션 #1 — 전산 「최근 사용」: 매일 여는 진입점 습관화)
- 전산 페이지 상단에 **「최근 사용」 섹션**(`#sec-recent-jeonsan`) 추가. 보험사 전산 카드를 클릭하면 localStorage(`ic_recent_jeonsan_v1`)에 기록되어, 다음 방문 시 **자주 가는 전산을 1탭으로 재접근**(최근순 6개·칩 UI·지우기). 설계사가 하루에도 여러 번 드나드는 전산을 "항상 인슈어커넥트를 거쳐서" 열게 만드는 습관 고리. 백엔드 변경 0(순수 클라이언트).
- "인슈어커넥트를 설계사 일상의 필수로" 5대 레버 중 1번(매일 진입 습관). 후속: 사례 네트워크·미니 CRM·모닝 브리핑·만기 푸시.
### Verified
- 인라인 `<script>` 33블록 구문 0오류 · record/render/CSS 배선 확인 · 보안 HIGH 0 · release.mjs

## [2.41.0] - 2026-06-10
### Fixed (카드뉴스 화질·가로형 표시 — 모바일에서 안 보이던 문제)
- **줌해도 흐리던 문제**: 원본은 **3200×1800 고화질 PNG**(서버도 다운스케일 없이 원본 서빙·업로드도 무압축)인데, v2.40.0에서 넣은 **`will-change: transform`이 이미지를 저해상도 래스터로 레이어에 고정**해 줌 시 원본에서 재래스터되지 못하고 뿌옇게 보였음. → **`will-change` 제거**(정지 줌에서 브라우저가 원본 3200px로 재렌더 → 선명) + **최대 줌 4→6배**(세부 표 글씨 판독).
- **가로 자료가 작게 뜨던 문제**: 모바일 카드뉴스 뷰포트가 **`aspect-ratio: 3/4`(세로 박스)**여서 16:9 가로 자료가 얇은 띠로 갇혀 작게 표시됨. → **`16/9`(가로 박스)로 변경** + 모달/스테이지 패딩 축소로 **화면 폭을 꽉 채움**.
### Verified
- 인라인 `<script>` 33블록 구문 0오류 · will-change 0건 · CN_ZMAX 6 · 16:9 적용 · 보안 HIGH 0 · release.mjs (+ 실브라우저 화질 확인)

## [2.40.0] - 2026-06-10
### Fixed (카드뉴스 팝업 줌 — 작은 글씨 확대 불가 문제)
- **근본원인**: 카드뉴스 모달이 `position:fixed` + 뷰포트단위(`max-height:92vh/540px`) + `.cn-viewport{overflow:hidden}` + 복사방지 guard 구조라, **모바일 네이티브 핀치줌이 모달 내용을 확대하지 못하고 모달만 시각뷰포트에 맞춰 재배치**됨("팝업이 리사이즈되는데 카드 내용은 확대 안 됨" 증상).
- **해결**: 모달 내부 **전용 줌**을 구현 — 카드 이미지에 직접 `transform: translate()+scale()` 적용. **핀치(두 손가락)·더블탭/더블클릭·마우스 휠(커서 기준)·＋/－ 버튼·드래그 팬** 지원(배율 1~4x, 팬 경계 클램프). 줌 안 했을 때 1지 스와이프는 기존대로 슬라이드 전환, 줌 상태에선 1지 드래그=이동. 슬라이드 전환·모달 재오픈 시 줌 초기화. **복사·캡처 방지 guard는 그대로 유지**(이미지 element에만 transform). PDF 카드는 iframe 자체 스크롤을 위해 네이티브 터치 허용.
- `.cn-viewport{touch-action:none}`(이미지 카드)로 제스처를 JS가 전담. 줌 사용법 힌트 2.6초 노출.
### Verified
- 인라인 `<script>` 33블록 구문 0오류 · 줌 함수/DOM 배선 확인 · 보안 HIGH 0 · release.mjs
...
- 사이드바 **빠른메뉴 박스를 로그인정보(계정) 카드와 동일 규격**(좌우 margin 12px·padding 12px·box-shadow)으로 정렬 — 폭/여백 일치.
### Verified
- 보안 HIGH 0 · release.mjs

## [2.37.0] - 2026-06-10
### Changed (빠른 메뉴 → 사이드바 docking)
- PC 우측 **⚡ 플로팅 빠른 메뉴(팝업/런처)를 좌측 사이드바 「로그인정보」 아래로 docking**. `.menu-pill` 디자인 그대로(뉴스·전산·GA·수납·소식지·청구·채용·강의·실무도구·삼따AI·파트너사). 우측 플로팅 팝업·⚡런처는 숨김. 네비게이션 가이드(투어) quickmenu 스텝이 사이드바 빠른메뉴를 가리키도록 `data-tour` 이전(데스크톱=사이드바 / 모바일=모바일섹션 자동 분기, `icTourFind` 가시성 기준).
### Verified
- `data-tour=quickmenu` 2개(사이드바·모바일) · 중복 id 0 · 플로팅 잔여 data-tour 0 · 보안 HIGH 0 · release.mjs

## [2.36.0] - 2026-06-10
### Changed (광고 가독성 + 홈 정리)
- **광고 팝업 확대**(텍스트 가독성↑): SEO 랜딩 자동 팝업 **380→520px**, 홈 배너 클릭 팝업 **420→560px**(이미지가 커져 광고 내 텍스트가 잘 보임). 모바일은 기존 풀폭 유지.
- **홈 광고배너 높이 = 옆 섹션(자유게시판·삼따AI)과 동일**: `align-self:stretch` + 다크 배경 + `object-fit:contain`(이미지의 어두운 배경과 블렌딩, 크롭 없음).
- **홈 즐겨찾기 플로팅 팝업 제거**(요청): 데스크톱 포함 전체 숨김. 즐겨찾기 기능(실무도구 ☆·실무도구 페이지)은 유지. 네비게이션 가이드(투어)의 즐겨찾기 스텝도 제거.
### Verified
- `node --check`(seo-cta) · 투어 배열 유효 · 보안 HIGH 0 · release.mjs

## [2.35.0] - 2026-06-10
### Changed (관리자 통합 기록지표 — 렌트카·통신을 「파트너사 통계」로 통합)
- 통합 기록지표 슬라이더 **8탭 → 6탭**. **「📊 클릭 통계」 → 「🤝 파트너사 통계」**로 변경하고 기존 **「🚗 렌트카 카드」·「📱 통신·휴대폰」 슬라이드를 이 탭으로 통합**(클릭 통계 + 렌트카 funnel + 통신 funnel 한 화면). 유입경로·회원지표 탭 재번호(6·7→4·5). 런타임 DOM 이동(기존 패널 이동 패턴 미러) + 차트 디스패치·로드 인덱스 갱신.
### Verified
- `ssGo`·이동블록 문법 검증 · 탭 6개·잔여 `ssGo(6/7)` 0 · 렌트카/통신 콘텐츠 소스 온전 · 보안 HIGH 0 · release.mjs

## [2.34.0] - 2026-06-10
### Changed (홈 광고 배너 → 팝업 확대 + 「상담 받아보기」 CTA)
- 홈 대시보드 제휴상품 광고 배너 클릭 동작 변경: **직접 외부이동 → 팝업(배너 확대 + 바로 아래 「💬 상담 받아보기」 버튼)**. 버튼이 네이버폼(`naver.me/xD8zNndZ`, 새 탭) 연결. 배너는 `<button>`(`openHomeAd`)으로 전환, 모달 X·바깥클릭·ESC 닫기. 전환 퍼널 추적: `배너클릭` → `상담클릭`(`trackCardClick('홈광고',…)`).
### Verified
- 모달 IIFE 문법 검증 · 마크업 배포 확인 · 보안 HIGH 0 · release.mjs
...
## [1.9.6] - 2026-05-18
- 정리: 홈 하단의 공개 이용·개인정보 안전·중립 정보 제공·피드백 안내 신뢰 카드 4개 제거 (푸터·사이드바와 중복)
