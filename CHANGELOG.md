...
- **📰 홈 보험뉴스 위젯(네이버식)**: 구글 뉴스 RSS(키·검수 불필요)에서 보험 키워드 뉴스를 실시간으로 가져와 홈 대시보드에 노출. 카테고리 탭(전체/손해보험/생명보험/정책=금감원) · 제목+출처+시간 · 클릭 시 새 탭 원문. **비로그인도 노출**(재방문 훅·유입)
- **`GET /api/news?cat=`**: 구글 뉴스 RSS fetch → `<b>`태그·HTML엔티티 제거·제목/언론사 분리 → **D1 30분 캐시**(외부 호출 최소화·속도). 카테고리 쿼리 코드 고정(변조 차단). 실패 시 stale 캐시·빈 결과 graceful
- **공유 유입(방향 A 1단)**: 각 뉴스에 「공유」 버튼 → 헤드라인+사이트 링크를 카톡방·네이버 카페/밴드에 1초 붙여넣기(navigator.share/클립보드) → 비회원 유입 통로
- **마이그레이션** `d1_v2_14_0_news.sql`(ic_news_cache, 원격 D1 적용 완료)
### Verified
- 구글뉴스 RSS 라이브 108건 수신 · `node --check` · 보안 HIGH 0 · 배포 후 `/api/news` 라이브 응답 확인
### Note
- 소스는 키 불필요한 구글 뉴스 RSS. 추후 네이버 검색 API 키 발급 시 소스만 교체 가능(UX 동일)
