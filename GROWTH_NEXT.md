...
> 현재 배포 버전: **v2.36.0**. 작업 전 `node scripts/release.mjs --dry`로 보안/버전 확인.
> 보류: 빠른메뉴를 사이드바(.side-account 아래)로 이동 — "빠른메뉴 팝업"의 데스크톱 정체가 코드상 불명확(즐겨찾기=#fav-float-popup만 확인). 사용자 확인 필요.
